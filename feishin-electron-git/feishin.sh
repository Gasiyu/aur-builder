...
_FLAGS_FILE="${XDG_CONFIG_HOME}/feishin-flags.conf"
declare -a flags
...
if [[ "${EUID}" -ne 0 ]] || [[ "${ELECTRON_RUN_AS_NODE}" ]]; then
    exec electron39 "${_RUNNAME}" "${_OPTIONS}" "${flags[@]}" "$@" || exit $?
else
    exec electron39 "${_RUNNAME}" "${_OPTIONS}" --no-sandbox "${flags[@]}" "$@" || exit $?
fi
